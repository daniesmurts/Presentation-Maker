...
export const register = (email: string, password: string, display_name: string) => client.post<{ user: User }>('/api/auth/register', { email, password, display_name }).then((r) => r.data.user)
