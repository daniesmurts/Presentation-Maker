...
export default function AdminOverviewPage() {
  ...
  return (
    <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
      ...
      {/* The demo funnel: how many minutes led to a plan, a click, an
          account. Plans + typed ≈ the second step; the ratio is the number. */}
      <Stat label={O.tryFunnel} value={`${data.try_7d.plan} → ${data.try_7d.registered}`}
            sub={`${data.try_7d.started + data.try_7d.typed} ${O.tryStarted} (${data.try_7d.typed} ${O.tryTyped}) · ${data.try_7d.plan} ${O.tryPlan} · ${data.try_7d.cta} ${O.tryCta} · ${data.try_7d.registered} ${O.tryRegistered}`} />
