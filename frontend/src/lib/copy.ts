...
export const copy = {
  ...
  auth: {
    ...
    forgot: {
      title: 'Восстановление пароля', hint: 'Укажите e-mail — пришлём ссылку для сброса пароля.',
      submit: 'Отправить ссылку', backToLogin: 'Вернуться ко входу',
    },
    reset: {
      title: 'Новый пароль', submit: 'Сохранить пароль',
      invalidToken: 'Ссылка недействительна или устарела — запросите новую.',
      success: 'Пароль изменён. Теперь войдите с новым паролем.', toLogin: 'Ко входу',
    },
    verify: {
      banner: 'Подтвердите e-mail — мы отправили ссылку на ', resend: 'Отправить письмо ещё раз',
  ...
  talk: {
    ...
    changeMenu:   'Переделать всё', changeHint: 'Всё выступление сразу: переписать по одному замечанию или нарисовать картинки', more: 'Ещё',
  ...
  rewrite: {
    button: 'Переписать всё', title: 'Переписать всё выступление', short: 'Одно замечание ко всем слайдам — старое и новое рядом',
  ...
  // Rehearsal («Репетиция»): speak the talk into the browser, get the time
  // per slide, what was said against the speaker's text, and the text
  // rewritten the way it was said. The words are recognised by the browser
  // itself — the copy says so where the microphone is asked for.
  rehearsal: {
    button: 'Репетиция', hint: 'Проговорите выступление: время по слайдам, что сказано и что пропущено, текст докладчика — так, как вы это сказали.',
    heading: 'Репетиция', start: 'Начать', finish: 'Завершить', finishing: 'Сохраняем…', cancel: 'Отменить', restart: 'Ещё раз',
    intro: 'Листайте слайды как на выступлении и говорите вслух. Слова распознаёт ваш браузер — звук никуда не отправляется, на сервер уходит только текст.',
    micAsk: 'Браузер спросит разрешение на микрофон.',
    noSpeech: 'В этом браузере нет распознавания речи — репетиция запишет только время по слайдам. Для разбора сказанного откройте Chrome, Яндекс Браузер, Edge или Safari.',
    micDenied: 'Микрофон не разрешён — репетиция идёт без распознавания, только время по слайдам. Разрешите микрофон в адресной строке и начните заново.',
    micMissing: 'Микрофон не найден — репетиция идёт без распознавания, только время по слайдам.',
    listening: 'Слушаем', paused: 'Микрофон выключен', keys: '→ следующий слайд · ← предыдущий · Esc — завершить',
    tooShort: 'Репетиция короче десяти секунд — нечего разбирать.',
    saved: 'Репетиция сохранена',
    // Report
    report: 'Разбор репетиции', of: (title: string) => `Репетиция · ${title}`,
    total: 'Всего', target: 'план', words: 'слов', wpm: 'слов в минуту', fillers: 'слов-паразитов',
    paceSlow: 'медленно — можно чуть живее', paceOk: 'хороший темп', paceFast: 'быстро — аудитория не успевает',
    overTime: 'дольше плана', underTime: 'короче плана', noTarget: 'Укажите длительность выступления, чтобы видеть план по слайдам.',
    perSlide: 'По слайдам', time: 'Время', coverage: 'Сказано',
    coverageLabel: { covered: 'Сказано', partial: 'Частично', skipped: 'Пропущено', no_speech: 'Молчание' } as Record<string, string>,
    missed: 'Не прозвучало', added: 'Добавили от себя', spoken: 'Как вы это сказали', transcript: 'Расшифровка',
    reviewCta: 'Разобрать сказанное', reviewing: 'Сравниваем с текстом докладчика — около минуты.',
    reviewLead: 'Что было сказано, что пропущено, и текст докладчика — так, как вы его произнесли.',
    reviewFree: (left: number) => left > 0 ? `На бесплатном тарифе — ${left === 1 ? 'один разбор' : `${left} разбора`} в месяц.` : 'Разбор на этот месяц использован. На тарифе Pro — без ограничения.',
    reviewFailed: 'Разбор не получился', retry: 'Попробовать ещё раз',
    summary: 'Итог', strengths: 'Сильные стороны', improvements: 'До выступления',
