...
const bullets = (items: string[], title = 'Слайд'): Slide =>
  ({ type: 'bullets', title, notes: 'длинный текст докладчика '.repeat(40), citations: [], body: { items } } as unknown as Slide)
...
describe('findOverfullSlides', () => {
  ...
  it('gives a cta slide a small budget — one ask, set large', () => {
    const cta = {
      type: 'cta', title: 'Что дальше', notes: '', citations: [],
      body: { action: 'Подпишите пилот', reasons: Array.from({ length: 6 }, (_, i) => `причина ${i}`), contact: null },
    } as unknown as Slide
    expect(findOverfullSlides([cta])).toHaveLength(1)
  })
...
describe('slideBodyText', () => {
  it('reads the visible text of each slide type', () => {
    ...
    const title = { type: 'title', title: 'Т', notes: '', citations: [], body: { subtitle: 'под', presenter: null } } as unknown as Slide
